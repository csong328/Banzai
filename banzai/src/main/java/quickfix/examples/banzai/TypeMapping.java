...
public class TypeMapping {
  static private final TwoWayMap<OrderSide, Side> sideMap = new TwoWayMap<>();
  static private final TwoWayMap<OrderType, OrdType> typeMap = new TwoWayMap<>();
  static private final TwoWayMap<OrderTIF, TimeInForce> tifMap = new TwoWayMap<>();

  public static Side sideToFIXSide(final OrderSide side) {
    return sideMap.getFirst(side);
  }

  public static OrderSide FIXSideToSide(final Side side) {
    return sideMap.getSecond(side);
  }

  public static OrdType typeToFIXType(final OrderType type) {
    return typeMap.getFirst(type);
  }

  public static OrderType FIXTypeToType(final OrdType type) {
    return typeMap.getSecond(type);
  }

  public static TimeInForce tifToFIXTif(final OrderTIF tif) {
    return tifMap.getFirst(tif);
  }

  public static OrderTIF FIXTifToTif(final TimeInForce tif) {
