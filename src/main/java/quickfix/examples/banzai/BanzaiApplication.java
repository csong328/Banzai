...
public class BanzaiApplication implements Application, IBanzaiService {
		...
		public void run() {
			try {
				...
				if (isAvailable) {
					...
					else if (message.getHeader()
							.isSetField(DeliverToCompID.FIELD)) {
	...
	private void executionReport(Message message, SessionID sessionID)
		...
		Order order = model
				.getOrder(message.getField(new ClOrdID()).getValue());
		if (order == null) {
			return;
		}
		...
		if (fillSize.compareTo(BigDecimal.ZERO) > 0) {
			...
			execution.setExchangeID(
					sessionID + message.getField(new ExecID()).getValue());
	...
	private void cancelReject(Message message, SessionID sessionID)
		...
		if (order.getOriginalID() != null)
			order = model.getOrder(order.getOriginalID());
	...
	public void send40(Order order) {
		quickfix.fix40.NewOrderSingle newOrderSingle = new quickfix.fix40.NewOrderSingle(
				new ClOrdID(order.getID()), new HandlInst('1'),
	...
	public quickfix.Message populateOrder(Order order,
